...
public class ImageController {
     ...
     * @param email 사용자 이메일 (프론트엔드에서 전달)
    ...
    public String generateImage(
        ...
        log.info("이미지 생성 요청 - Prompt: {}, Email: {}", prompt, email);

        // *** 로그인 상태 확인 및 User 객체 추가 ***
        mainController.checkLogin(model);

        try {
            String s3Key = imgService.generateImage(prompt, StringUtils.hasText(email) ? email : "비회원");
